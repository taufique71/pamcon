...
SetPartition SetPartition::LoadClusterListFile(const char* filename)
{
    string line;
    ifstream fin(filename);

    vector<string> lines;
    vector< vector<int> > clustLst;
    int nItem = 0;
    if (fin.is_open())
    {
        while ( getline (fin, line) )
        {
            ...
            do {
                ...
                number[idx++] = '\0';
                clust.push_back(atoi(number));
            } while(0 != *str++);
            clustLst.push_back(clust);
            nItem = nItem + clust.size();
        }
        fin.close();
    }

    vector<int> v(nItem);
    for (int i = 0; i < clustLst.size(); i++){
        for(int j = 0; j < clustLst[i].size(); j++){
            v[clustLst[i][j]] = i+1;
        }
    }
...
/* 
 * Not part of CONPAS originally.
 * Function added by Taufique.
 * Writes cluster to file as a cluster list.
 * Elements are written as 0-based index, instead of default 1-based index in CONPAS
 * */
void SetPartition::WriteClusterListFile(string fname){
    ofstream outfile;
    outfile.open(fname, ofstream::trunc);
    for(auto i = 1; i < blocks.size(); i++){
        //printf("Block %d\n", i);
        for (auto j = 0; j < blocks[i].size(); j++){
            outfile << blocks[i][j]-1 << " ";
        }
        outfile << "\n";
    }
