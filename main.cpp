...
int main(int argc, char* argv[]){
    ...
    vector< string > clust_lst_fnames(k);
    for (int i = 0 ; i < k; i++){
        input_clusterings[i].resize(n);
        string clust_lst_fname = input_clustering_prefix + std::string(".") + to_string(i);
        clust_lst_fnames[i] = clust_lst_fname;
        read_clust_lst(clust_lst_fname, input_clusterings[i]); // Read cluster list and convert to cluster assignment
    }
    t1 = omp_get_wtime();
    ...
    * Based on connected component and pre-process threshold
    * */
    ...
    for(int i = 0; i < k; i++){
        for (int j = i; j < k; j++){
            ...
            //printf("%d clusters in %d; %d clusters in %d; dist(%d, %d): %0.2lf\n", ki, i, kj, j, i, j, (dist));
        }
    }
    ...
    for(int c = 0; c < n_components; c++){
        ...
        if ( (alg == "boem") || (alg == "saoem") ){
            ...
            for (int i = 0; i < k; i++){
                if ( VECTOR(membership)[i] == cid ){
                    fileNames.push_back(clust_lst_fnames[i]);
                }
            }
