...
public:
    ...
    COO(RIT nrows, CIT ncols, size_t nnz, pvector<RIT>* nzRows, pvector<CIT>* nzCols, pvector<VT>* nzVals, bool isWeighted): nrows_(nrows), ncols_(ncols), nnz_(nnz), sortType_(UNSORTED), isWeighted_(isWeighted) {
        nzRows_ = std::move(*(nzRows));
        nzCols_ = std::move(*(nzCols));
